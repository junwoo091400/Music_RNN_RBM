...
def rnnrbm(k_input = 15):
	...
	bu  = tf.Variable(tf.zeros([1, n_hidden_recurrent]), name="bu") # Bias for 'hidden_recurrent' -> next 'hidden_recurrent' [RNN]
	...
	def generate(num, x=x, size_bt=size_bt, u0=u0, n_visible=n_visible, prime_length=100, k_in = 15):
		"""
		...
		"""
		Uarr = tf.scan(rnn_recurrence, x, initializer=u0)# Generate: Hidden state Array from 'x(input)' and initializer 'u0'
		U = Uarr[np.int32(np.floor(prime_length/midi_manipulation.num_timesteps)), :, :] # acts as initial-hiddenstate....
		[_, _, _, _, _, music, _] = tf.while_loop(lambda count, num_iter, *args: count < num_iter,
